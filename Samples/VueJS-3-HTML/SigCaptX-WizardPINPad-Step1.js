...
function step1()
{
  ...
  //vue_app.userMsg("At start of step1() Cancel Button fore color is " + display_1.cancelButton.fontForeColor);
  ...
  if(wgssSignatureSDK.ResponseStatus.OK == status)
  {
    switch(id) 
    {
      case "input":
        ...
        {
          case 4: 
            break; //vue_app.userMsg("min chars entered")
          case 5: 
            break; //vue_app.userMsg("max chars entered")
          case 6: 
            break; //vue_app.userMsg("attempted to exceed min/max chars")
          default: 
            vue_app.userMsg("Input unexpected type: " + Type); 
            break;
        }
        break;
    ...
    case buttonEvent.CANCEL:
      vue_app.userMsg("Cancel");
      vue_app.script_Completed(true);
      break;
    default:
      vue_app.userMsg( "Exception: step1_Handler(): " + "unexpected event: " + id);
      break;
    }  
  }
  else
...
function onInputObjGetText(inputObjV, text, status) 
{
  if(wgssSignatureSDK.ResponseStatus.OK == status) 
  {
    vue_app.userMsg("Code entered: " + text);
    vue_app.script_Completed(true);
  }
  else
  {
    vue_app.userMsg("InputObj GetText error: " + status);
    vue_app.script_Cancelled();
  }
}