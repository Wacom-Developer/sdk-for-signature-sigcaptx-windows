...
{ 
  ...
  //vue_app.userMsg("Setting up previous button for screen 1");
  ...
  {
    //vue_app.userMsg("Defining step1Rectangle for pad " + pad.Range);
...
function screen_Display3(pad, buttonTextSource)
{
  ...
  //vue_app.userMsg("Setting up XMark on screen display 3 for pad : " + pad.Range);
  ...
  //vue_app.userMsg("Setting who position for " + pad.Range + " to " + this.who.xPos + " " + this.who.yPos);
  ...
  //vue_app.userMsg("Setting up button contents on screen display 3 for pad : " + pad.Range);
  
  // Set up the source of the button depending on what the user has selected on the HTML document
 //vue_app.userMsg("Setting up buttons from source: " + buttonTextSource);
  ...
  // Set up the source of the button depending on what the user has selected on the HTML document
 //vue_app.userMsg("Setting up Clear button from source: " + buttonTextSource);
  switch (buttonTextSource)  
  {
    case textSource.UTF8:
...
function setupNextButton(pad, buttonTextSource)
{
 ...
 //vue_app.userMsg("Setting up buttons for button text source " + buttonTextSource);
...
function setupCancelButton(pad, buttonTextSource, buttonFunc)
{
  ...
  // Set up the Cancel button object
  //vue_app.userMsg("setupCancelButton " + pad + " " + buttonTextSource + " " + buttonFunc);
  ...
  // Set up the source of the button depending on what the user has selected on the HTML document
  //vue_app.userMsg("Setting up buttons from source: " + buttonTextSource + " using prefix " + imageFilePrefix);
  
  switch (buttonTextSource)  
  {
    case textSource.UTF8:
      //vue_app.userMsg("UTF8");
      cancelButton.buttonText = "取消";
      break;
      
    case textSource.LOCAL:
      // Override the positions of the buttons when using images
      //vue_app.userMsg("LOCAL");
      cancelButton.xPos = "left";
      cancelButton.yPos = "bottom";
      // vue_app.userMsg("Setting up image file for cancel button using range " + pad.Range + " and prefix " + imageFilePrefix);
      cancelButton.imageFile = setButtonImageFile(buttonTextSource, pad.Range, imageFilePrefix);
     //vue_app.userMsg("Cancel button image file is " + cancelButton.imageFile);
      break;
      
    case textSource.REMOTE:
      //vue_app.userMsg("REMOTE");
      // Override the positions of the buttons when using images
      cancelButton.xPos = "left";
      cancelButton.yPos = "bottom";
      cancelButton.imageFile = setButtonImageFile(buttonTextSource, pad.Range, imageFilePrefix);
...
function setupPreviousButton(pad, buttonTextSource)
{
  ...
  // Set up the source of the button depending on what the user has selected on the HTML document
 //vue_app.userMsg("Setting up previous button from source: " + buttonTextSource);
  switch (buttonTextSource)  
  {
    case textSource.UTF8:
...
function setButtonImageFile(buttonTextSource, currentPadRange, imagePrefix)
{
  ...
  //vue_app.userMsg("setButtonImageFile: " + buttonTextSource + " "+ currentPadRange + " " + imagePrefix);
  ...
  //vue_app.userMsg("filePath: " + filePath);
