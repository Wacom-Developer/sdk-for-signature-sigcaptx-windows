...
import { buttonFunction, ButtonObject, CHECKBOX_USETICKSYMBOL, CheckboxObject, checkSize, checkSizeSelection, LineObject, OUTLINE, padColors, padRange, padType, RadioObject, RectangleObject, SOLIDLINE, TextObject, textSource, userMessage } from '../src/components/WizUtils.js';

export function STUPadDefs (padWidth, padHeight, checkBoxSize)
{
  userMessage("Defining pad for width/height : " + padWidth + "/" + padHeight);
  this.Font = "Verdana";
...
export function ScreenDisplay1 (pad, buttonTextSource)
{
...
export function ScreenDisplay2 (pad, buttonTextSource)
{
...
export function ScreenDisplay3 (pad, buttonTextSource)
{
  ...
  switch (buttonTextSource)
  {
    ...
    case textSource.REMOTE:
      ...
      this.okButton.imageFile = setButtonImageFile(pad.Range, "Accept");
      break;
  }
  ...
  switch (buttonTextSource)
  {
    ...
    case textSource.REMOTE:
      ...
      this.clearButton.imageFile = setButtonImageFile(pad.Range, "Clear");
      break;
  }
...
const setupNextButton = (pad, buttonTextSource) =>
{
  // Set up the Next button object
  const nextButton = new ButtonObject();
  nextButton.xPos = pad.xButtonRight;
  nextButton.yPos = pad.yButton;
  nextButton.buttonSize = pad.ButtonSize;
  nextButton.width = pad.ButtonWidth;
  nextButton.buttonBold = pad.ButtonBold;
  nextButton.buttonType = "Next";
  ...
  {
    nextButton.fontForeColor = "";
    nextButton.fontBackColor = "";
  }
  ...
  switch (buttonTextSource)
  {
    ...
    case textSource.REMOTE:
      ...
      nextButton.imageFile = setButtonImageFile(pad.Range, "RightArrow");
      break;
  }
  return nextButton;
};
...
const setupCancelButton = (pad, buttonTextSource, buttonFunc) =>
{
  let imageFilePrefix;

  // Set up the Cancel button object
  const cancelButton = new ButtonObject();
  ...
  switch (buttonTextSource)
  {
    ...
    case textSource.REMOTE:
      ...
      cancelButton.imageFile = setButtonImageFile(pad.Range, imageFilePrefix);
      break;
  }
  return cancelButton;
};
...
const setupPreviousButton = (pad, buttonTextSource) =>
{
  // Set up the Previous button object
  const previousButton = new ButtonObject();
  ...
  switch (buttonTextSource)
  {
    ...
    case textSource.REMOTE:
      ...
      previousButton.imageFile = setButtonImageFile(pad.Range, "LeftArrow");
      break;
  }
...
const setupContinueText = (pad, buttonTextSource) =>
{
  const nextToContinue = new TextObject();
...
// Define the image file which is used for a given button
function setButtonImageFile (currentPadRange, imagePrefix)
{
  let imageFile;

  const filePath = "http://gsdt.wacom.eu/SigCaptX/images/";
